...
import QC.Common (parseBS, toStrictBS)
...
take n s = maybe (L.length s < fromIntegral n) (== B.take n (toStrictBS s)) $
           parseBS (P.take n) s

takeByteString :: L.ByteString -> Bool
takeByteString s = maybe False (== toStrictBS s) . parseBS P.takeByteString $ s

takeLazyByteString :: L.ByteString -> Bool
takeLazyByteString s = maybe False (== s) . parseBS P.takeLazyByteString $ s
